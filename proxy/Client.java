...
public class Client implements Proxy {
	...
	/*@SubscribeEvent
	public void renderEvent(RenderLivingEvent event) {
		System.out.println("Rendering event");
	}*/
	...
	public void NameRenderEvent(Specials.Pre event) {
		if (event.entity instanceof EntityPlayer) {
			...
			if (canRenderName(event.renderer.func_177068_d(), event.entity)) {
				...
				float f = event.entity.isSneaking() ? event.renderer.NAME_TAG_RANGE_SNEAK
						: event.renderer.NAME_TAG_RANGE;

				if (d3 < (double) (f * f)) {
					String s = event.entity.getDisplayName().getFormattedText();
					if (NamesUI.DYNUsernames.containsKey(event.entity.getName())){
						s = s.replace(event.entity.getName(), NamesUI.DYNUsernames.get(event.entity.getName()));
					}
