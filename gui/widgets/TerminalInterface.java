package com.dyn.render.gui.widgets;
...
public class TerminalInterface extends GuiWidget {
	...
	public TerminalInterface(int x, int y, int termWidth, int termHeight, IComputerContainer computer) {
		...
		if (terminal != null) {
			...
			synchronized (terminal) {
				...
				for (int line = 0; line < th; line++) {
					...
					fontRenderer.drawString(text, x, y, colour, backgroundColour, 0, 0, greyscale);
					if ((tblink) && (ty == line)) {
						if ((tx >= 0) && (tx < tw)) {
			...
			if (m_focus) {
				...
				if ((!m_locked) && (computer != null) && (computer.isColour()) && (button >= 0) && (button <= 2)) {
					...
					if (term != null) {
						...
						computer.queueEvent("mouse_click", new Object[] { Integer.valueOf(button + 1),
		...
		if ((mouseX >= x) && (mouseX < (x + getWidth())) && (mouseY >= y) && (mouseY < (y + getHeight()))
			...
			if (term != null) {
				...
				if ((m_lastClickButton >= 0) && (!Mouse.isButtonDown(m_lastClickButton))) {
					if ((m_focus) && (!m_locked)) {
						computer.queueEvent("mouse_up", new Object[] { Integer.valueOf(m_lastClickButton + 1),
								Integer.valueOf(charX + 1), Integer.valueOf(charY + 1) });
					}
					m_lastClickButton = -1;
				}
				...
				if ((m_focus) && (!m_locked)) {
					if (wheelChange < 0) {
						computer.queueEvent("mouse_scroll", new Object[] { Integer.valueOf(1),
								Integer.valueOf(charX + 1), Integer.valueOf(charY + 1) });
					...
					if ((m_lastClickButton >= 0) && ((charX != m_lastClickX) || (charY != m_lastClickY))) {
						computer.queueEvent("mouse_drag", new Object[] { Integer.valueOf(m_lastClickButton + 1),
								Integer.valueOf(charX + 1), Integer.valueOf(charY + 1) });
